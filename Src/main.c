...
void EXTI0_IRQHandler(void){
	...
	x=SPI_LIS3DSH_Rx(AXIS_X);
	y=SPI_LIS3DSH_Rx(AXIS_Y);
	z=SPI_LIS3DSH_Rx(AXIS_Z);

	status_1=SPI_LIS3DSH_Rx(FIFO_SRC);

	if(x<-10) HAL_GPIO_WritePin(GPIOD,GPIO_PIN_12,1);
	else HAL_GPIO_WritePin(GPIOD,GPIO_PIN_12,0);

	if(x>10) HAL_GPIO_WritePin(GPIOD,GPIO_PIN_14,1);
	else HAL_GPIO_WritePin(GPIOD,GPIO_PIN_14,0);

	if(y<-10) HAL_GPIO_WritePin(GPIOD,GPIO_PIN_15,1);
	else HAL_GPIO_WritePin(GPIOD,GPIO_PIN_15,0);

	if(y>10) HAL_GPIO_WritePin(GPIOD,GPIO_PIN_13,1);
